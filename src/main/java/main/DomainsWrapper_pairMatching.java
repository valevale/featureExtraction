...
public class DomainsWrapper_pairMatching {
	 ...
	 * */
	public static int getSegmentsFrom_serverNEW(WebPageDocument firstDocument,
		...
		List<Tuple2<Segment, List<Tuple2<Segment,Float>>>> segment2relevantSegments_primaPersona =
				finder.findTopSegments_new(indexPathDominio1, firstDocument, secondDocument);
		...
		List<Tuple2<Segment, List<Tuple2<Segment,Float>>>> segment2relevantSegments_secondaPersona =
				finder.findTopSegments_new(indexPathDominio2, thirdDocument, fourthDocument);
		...
		for (int i=0; i<segment2relevantSegments_secondaPersona.size(); i++) {
			Segment currentSegment = segment2relevantSegments_secondaPersona.get(i)._1();
...
//		double threshold = Configurator.getCosSimThreshold();
		...
		if (segment2relevantSegments_primaPersona.size() != 0 && segment2relevantSegments_secondaPersona.size() != 0) {
...
//			SegmentSearcher searcher = new SegmentSearcher(indexPathDominio1);
			for (int j=0; j<segment2relevantSegments_primaPersona.size(); j++) {
				Segment seg = segment2relevantSegments_primaPersona.get(j)._1();
				List<Tuple2<Segment,Float>> relSeg2score_d2_primaPersona =
						segment2relevantSegments_primaPersona.get(j)._2();
				//				TopDocs hits = segment2relevantSegments_primaPersona.get(j)._2();
				//				for(ScoreDoc scoreDoc : hits.scoreDocs) {
				for (int i=0;i<relSeg2score_d2_primaPersona.size();i++) {
					Tuple2<Segment,Float> relSeg_score = relSeg2score_d2_primaPersona.get(i);
					Segment seg_secondDocument = relSeg_score._1();
					float score = relSeg_score._2();
					...
					id_found = addLink_CCconDoppiaCacheAndEfficienza(seg, seg_secondDocument, thirdDocument, fourthDocument, score,
							relevantSegments_thirdDocument, relevantSegments_fourthDocument,
							segment2relevantSegments_secondaPersona, indexPathDominio2, id_found,
							xpath2isIdentificativa_d1,xpath2isIdentificativa_d2);
		...
		if (!id_found) {
			...
			int countMatching_p1 = countMatchings_new(segment2relevantSegments_primaPersona);
			int countMatching_p2 = countMatchings_new(segment2relevantSegments_secondaPersona);
...
//	} //fine main
	...
	private static boolean addLink_CCconDoppiaCacheAndEfficienza(Segment firstSegment, Segment secondSegment,
			...
			List<Tuple2<Segment, List<Tuple2<Segment,Float>>>> segment2hits_secondaPersona,
		...
		if (genericXpath_firstSegment != null && genericXpath_secondSegment != null) {
			...
			if (isARelevantMatching_new(genericXpath_firstSegment.getXpath(), doc3, 
...
//	}
	...
	private static boolean isARelevantMatching_new(String xpath1, WebPageDocument doc1,
			String xpath2,WebPageDocument doc2,
			List<Tuple2<Segment, List<Tuple2<Segment,Float>>>> segment2hits, String indexPath)
					throws XPathExpressionException, IOException, ParserConfigurationException {
		...
		for (int i=0;i<segment2hits.size();i++) {
			Tuple2<Segment,List<Tuple2<Segment,Float>>> segment2hit = segment2hits.get(i);
...
//				TopDocs hits = segment2hit._2();
				List<Tuple2<Segment,Float>> hits = segment2hit._2();
				for(Tuple2<Segment,Float> hit : hits) {
					Segment seg_secondDocument = hit._1();
...
//					}
	...
	public static int countMatchings_new(List<Tuple2<Segment, List<Tuple2<Segment,Float>>>> segment2hits) {
		int count = 0;
		for (int j=0; j<segment2hits.size(); j++) {
			List<Tuple2<Segment,Float>> hits = segment2hits.get(j)._2();
			//per ogni risultato
